...
- 투표 시간은 최대 60초이며, 모든 생존 플레이어가 투표를 완료하면 즉시 종료된다.
- 마피아 정답 입력 시간은 20초다.
- 채팅은 대기방(waiting)과 투표 단계(voting)에서만 가능하다. 그림 그리는 단계(playing)에서는 채팅이 비활성화된다.
...
- `voting`: 생존 플레이어가 마피아로 의심되는 대상을 투표한다 (최대 60초, 전원 투표 시 즉시 종료, **채팅 가능**)
